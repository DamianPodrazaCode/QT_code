...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent), ui(new Ui::MainWindow) {
    ...
    // maksymalna ilość wątków jak może być użyta, ale także poszeżenie jeżeli fizycznie braknie wątków
    threadPool->setMaxThreadCount(5);
    // czas wygaśnięcia nieużywanego wątku
...
void MainWindow::on_pb_th1_toggled(bool checked) {
    if (checked) {
        th1_run = true;
        thread1 = QtConcurrent::run(threadPool, thread1Fun, this);
...
void MainWindow::on_pb_th2_toggled(bool checked) {
    if (checked) {
        th2_run = true;
        thread2 = QtConcurrent::run(threadPool, thread2Fun, this);
...
void MainWindow::on_pb_th3_toggled(bool checked) {
    if (checked) {
        th3_run = true;
        thread3 = QtConcurrent::run(threadPool, thread3Fun, this);
...
void MainWindow::on_pb_th4_toggled(bool checked) {
    if (checked) {
        th4_run = true;
        thread4 = QtConcurrent::run(threadPool, thread4Fun, this);
...
void MainWindow::on_pb_th5_toggled(bool checked) {
    if (checked) {
        th5_run = true;
        thread5 = QtConcurrent::run(threadPool, thread5Fun, this);
...
void MainWindow::update_l_th1(QString str) { ui->l_th1->setText(str); }
void MainWindow::update_l_th2(QString str) { ui->l_th2->setText(str); }
void MainWindow::update_l_th3(QString str) { ui->l_th3->setText(str); }
void MainWindow::update_l_th4(QString str) { ui->l_th4->setText(str); }
void MainWindow::update_l_th5(QString str) { ui->l_th5->setText(str); }
