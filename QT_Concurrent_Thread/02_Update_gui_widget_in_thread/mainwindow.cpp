...
MainWindow::~MainWindow() {
    ...
    ConcurentThread.waitForFinished(); // oczekiwanie na zakończenie wątku, jak tego nie będzie to wątek pozostanie uruchomiony nawet po zamknięciu aplikacji
    delete ui;
}
...
void MainWindow::on_pb_thread_toggled(bool checked) {
    if(checked) {
        thread_run = true;
        // podpięcie i uruchomienie ,funkji wątku, pierwszy par. to nazwa funkcji, drugi i następne to refer. argumentów funkcji
        // funkcja jest urruchamiana raz, później wychodzi i umiera, więc o cykliccznoś w wewnątrz trzeba zadbać samemu
