...
Perc2in::Perc2in(QObject* parent) : QObject{parent} {
    w1 = (QRandomGenerator::global()->bounded(100)) / 100.0;
    w2 = (QRandomGenerator::global()->bounded(100)) / 100.0;
    wp = (QRandomGenerator::global()->bounded(100)) / 100.0;
}
...
void Perc2in::showState() {
    ...
    qInfo() << "w1 -> " << w1 << ", w2 ->" << w2 << ", ws ->" << wp << ", out ->" << out << "\n";
}
